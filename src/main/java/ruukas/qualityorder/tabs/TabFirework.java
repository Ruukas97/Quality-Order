...
public class TabFirework extends Tab {
					...
					if (starCompound.hasKey("Explosion")) {
						...
						if (starExplosionCompound.hasKey("Colors")) {
							if (currentRocket.getTagCompound().hasKey("Fireworks")) {
								...
								NBTTagList rocketExplosionsTagList = rocketFireworksCompound.getTagList("Explosions",
										Constants.NBT.TAG_COMPOUND);
								for (int j = 0; j < rocketExplosionsTagList.tagCount(); ++j) {
									rocketExplosionsTagList.getCompoundTagAt(j).setIntArray("FadeColors",
											starExplosionCompound.getIntArray("Colors"));
									;
								}
							}
						}
